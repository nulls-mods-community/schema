from generate import generate, CSV_DIRECTORY_NAME, GENERATED_DIRECTORY_NAME, ENHANCED_DIRECTORY_NAME
...
def _main() -> None:
    ...
    try:
        generate(base / CSV_DIRECTORY_NAME, temp_build_dir / GENERATED_DIRECTORY_NAME, base / ENHANCED_DIRECTORY_NAME)

        for schema_file in temp_build_dir.glob("**/*.schema.json"):
            _minify_schema(schema_file, build_dir / schema_file.relative_to(temp_build_dir))
    finally:
        shutil.rmtree(temp_build_dir, ignore_errors=True)
        temp_build_dir.unlink(missing_ok=True)

    print(f"✅ Done! The minified schema build is available in the {BUILD_DIRECTORY_NAME}/ directory.")
